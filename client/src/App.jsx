...
function App() {
  ...
  const [sleepSound, setSleepSound] = useState(null);
  const [sleepVolume, setSleepVolume] = useState(0.6);
  ...
  const sleepGainRef = useRef(null);
  const sleepNodesRef = useRef([]);
  ...
  useEffect(() => {
    if (sleepGainRef.current && audioCtxRef.current) {
      sleepGainRef.current.gain.setTargetAtTime(sleepVolume, audioCtxRef.current.currentTime, 0.05);
    }
  }, [sleepVolume]);

  useEffect(() => {
    if (!sleepSound || !sleepTimerEndsAt) {
      setSleepTimerLeft(null);
      return undefined;
    }
    const tick = () => setSleepTimerLeft(Math.max(0, Math.round((sleepTimerEndsAt - Date.now()) / 1000)));
    tick();
    const id = window.setInterval(tick, 1000);
    return () => window.clearInterval(id);
  }, [sleepSound, sleepTimerEndsAt]);

  useEffect(() => {
    if (view !== 'sleepSounds' && sleepNodesRef.current.length > 0) {
      stopSleepSound();
    }
  }, [view]);
  ...
  const stopSleepSound = () => {
    sleepNodesRef.current.forEach((node) => {
  ...
  const armSleepTimer = (minutes) => {
    ...
    setSleepTimerEndsAt(Date.now() + minutes * 60000);
    sleepTimeoutRef.current = setTimeout(() => stopSleepSound(), minutes * 60000);
  };

  const chooseSleepTimer = (minutes) => {
    setSleepTimerMinutes(minutes);
    if (sleepSound) armSleepTimer(minutes);
  };
  ...
  const playSleepSound = async (soundId) => {
    ...
    sleepNodesRef.current = buildSleepGraph(ctx, soundId, sleepGainRef.current);
    setSleepSound(soundId);
    armSleepTimer(sleepTimerMinutes);
  };
    ...
    <div className="landing-shell">
      ...
      <main className="landing-grid">
        ...
        <section className="card path-card">
          ...
          <button className="primary-btn" onClick={() => setView('sleepSounds')}>Open sleep sounds</button>
        </section>
        <section className="card path-card">
    ...
    <section className="card">
      ...
      <div className="sound-grid">
        {SLEEP_SOUNDS.map((sound) => (
          <button
            key={sound.id}
            className={`sound-card ${sleepSound === sound.id ? 'playing' : ''}`}
            onClick={() => (sleepSound === sound.id ? stopSleepSound() : playSleepSound(sound.id))}
          >
            <span className="sound-emoji">{sound.emoji}</span>
            <span className="sound-name">{sound.name}</span>
            <span className="small">{sound.description}</span>
            {sleepSound === sound.id && <span className="sound-playing-tag">▶ Playing — tap to stop</span>}
          </button>
        ))}
      </div>
        ...
        <div className="form-group" style={{ marginBottom: 0 }}>
          ...
          <div className="timer-options">
            {[0, 15, 30, 60].map((minutes) => (
              <button
                key={minutes}
                className={`timer-btn ${sleepTimerMinutes === minutes ? 'selected' : ''}`}
      ...
      {sleepSound && sleepTimerLeft !== null && (
        <p className="small" style={{ marginTop: 14 }}>Sound stops in {formatSeconds(sleepTimerLeft)}.</p>
      )}
      {sleepSound && (
        <div className="actions">
          <button className="secondary-btn" onClick={stopSleepSound}>Stop sound</button>
        </div>
      )}
    </section>
  );
  ...
  return (
    <div className="app-shell">
      ...
      {view === 'sleepSounds' && renderSleepSounds()}
    </div>
  );
}
